...
class StickHeroGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    lazy var walkAction:SKAction = {
        ...
        for i in 0...1 {
            let texture = SKTexture(imageNamed: "boy\(i + 1).png")
...
private extension StickHeroGameScene {
    func loadBackground() {
        guard let _ = childNode(withName: "background") as! SKSpriteNode? else {
            let texture = SKTexture(image: UIImage(named: "DragonBoyBg.jpg")!)
    ...
    func loadHero() {
        let hero = SKSpriteNode(imageNamed: "boy1")
    ...
    func loadTip() {
        ...
        tip.text = "Place your hand on the screen"
    ...
    func loadPerfect() {
        defer {
            ...
            let scale = SKAction.sequence([SKAction.scale(to: 1.4, duration: 0.5), SKAction.scale(to: 1, duration: 0.3)])
